...
class Auth(object):
    ...
    def app_sign(self, bucket, cos_path, expired, upload_sign=True):
        appid = self.cred.appid
        bucket = bucket
        secret_id = self.cred.secret_id
        now = int(time.time())
        rdm = random.randint(0, 999999999)
        cos_path =  parse.quote(cos_path.encode(), '~/')
        if upload_sign:
        ...
        secret_key = self.cred.secret_key.encode('ascii')
        ...
        print(cos_path, plain_text, secret_key, sign_base64)
        return sign_base64
